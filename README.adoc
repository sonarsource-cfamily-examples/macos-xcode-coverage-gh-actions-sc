...
*This project is analysed on https://sonarcloud.io/dashboard?id=sonarsource-cfamily-examples_macos-xcode-coverage-gh-actions-sc[SonarCloud]!*
...
It is very easy to analyze a C, C++ and Objective-C project on SonarCloud with code coverage and https://docs.sonarcloud.io/getting-started/github/[GitHub Actions]:
...
.. Merge the multiple `\*.xcresult` into a single `*.xcresult` file with the help of `xcresulttool merge`
.. Run link:xccov-to-generic.sh[xccov-to-generic.sh] to produce a https://docs.sonarcloud.io/enriching/test-coverage/generic-test-data/[Generic Test Data] format that is compatible with SonarCloud
